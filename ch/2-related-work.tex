...
\subsection{Two views of the dataset bias problem}\label{two-views-of-the-dataset-bias-problem}
...
However, the key contribution is the objective functions for the adversary.
...
In this spirit, \citet{kilbertus2017avoiding} use causal graphs to develop new fairness criteria.
In their work, the prediction output \(\hat{y}\) is a part of the causal graph that models the data.
\citet{hardt2016equality} had already defined
Demographic Parity and Equalized Odds (with Equality of Opportunity as a special case)
as statistical independencies;
Demographic Parity: \(s \bot \hat{y}\), Equalized Odds: \(s \bot \hat{y} | y\).
In causal graphs, the first is equivalent to no existing path between \(s\) and \(\hat{y}\),
and the second independence allows paths to exist between \(s\) and \(\hat{y}\)
as long as all paths are all blocked by \(y\).
Instead of considering paths that start from \(s\) directly,
...
The reason for this seems to be that if all influence of \(s\) on \(\hat{y}\) was removed,
only very few features would remain to make a decision
because usually nearly all features are influenced by \(s\) somehow.

The concept of removing causal influence
can be efficiently expressed with \emph{interventions} on the causal graph
(level 2 according to Pearl).
...
The causal model that is used in the paper is a structural equation model.
Each edge in the graphical model corresponds to an equation.
There is a node for each input feature, each sensitive attribute (usually only one),
each proxy and the output of the classifier.
Additionally, there is an unobserved error variable for each input feature (and each proxy).
The equation that defines the classification result in terms of the inputs is parameterized.
This equation represents the classifier \(\hat{y} = g_{\theta}(x,p,s)\).
In the paper, only linear functions are used in this equation;
meaning that the classifier is linear.

The following is then the procedure to construct a fair classifier:

\begin{enumerate}
  \item
    Do an intervention with \(do(p=p')\). Change the structural equations accordingly.
  \item
    Construct one big equation for the classifier output \(g_{\theta}(x,p',s) = ...\)
    by substituting all variables according to the graph up to the root nodes,
    taking into account the \(do\)-intervention.
  \item
    Find the set of values of \(\theta\) that fulfills the fairness criterion:
    \(g_{\theta}(x,p'=0,s) = g_{\theta}(x,p'=1,s)\).
    If there is no such value for \(\theta\), the procedure does not work.
  \item
    Within the allowed range of values for \(\theta\),
    find the one that minimizes the classification loss.
\end{enumerate}

The simplest way to achieve step 3 is usually to choose a \(\theta\)
which makes all contributions from descendants of \(p\) vanish,
but the procedure also allows for more sophisticated ways to remove the influence of \(p\).
(Descendants of \(p\) are all variables that can be reached from \(p\) via directed paths.)

% \includegraphics{\{\{ site.url \}\}/blog/assets/fairness-literature/all-connected.svg}
% \includegraphics{\{\{ site.url \}\}/blog/assets/fairness-literature/p-separated.svg}

The resulting classifier is fair by construction.
...
There are no experiments presented in the paper.
...
True individual causal fairness is, however, the goal of \citet{kusner2017counterfactual}.
In their paper, a new fairness criterion which they call ``counterfactual fairness''
is defined making use of causal reasoning.
For the definition, they assume that we have a causal model
that can be used to compute counterfactuals (corresponding to level 3 in Pearl's hierarchy).
This is in contrast to \citet{kilbertus2017avoiding} who relied on interventions (level 2).
...
This is because those criteria are statistical criteria.
...
The authors prove that \(\hat{y}\) is counterfactually fair
if it does not causally depend (as defined by the causal model) on \(s\) or any descendants of \(s\).
This motivates the following algorithm to train a fair predictor.
We assume that we have the causal model and the prior over the background variables \(U\).
We train an classifier \(g_{\theta}(U, x_{\not\succ s})\) that is parameterized by \(\theta\).
\(x_{\not\succ s}\) refers to all features \(x\) that are \emph{not} descendants of \(s\).
Furthermore, we have a loss function that measures how well the class labels \(y\) are predicted.
For training, we optimize the expected loss over \(P(U|x_i, s_i)\)
which is the distribution over the background variables, as defined by the causal model,
given training instances \(x_i\) and \(s_i\).
Note that the number of features that are non-descendants of \(s\) might be very small or even zero.
However, this is not a problem as the classifier \(g_{\theta}(U, x_{\not\succ s})\)
can get the necessary information from \(U\).
The variables in \(U\) are, by definition, non-descendants of \(s\).
Predictions are made via \[\hat{y} = \mathbb{E} [g_{\theta}(U^*, x_{\not\succ s}^*)|x^*, s^*]\].

Constructing the causal model remains the major weakness of this approach.
The authors point to existing literature on the topic and give some additional vague guidelines.
In the experiment section, the algorithm is applied to the Law School Success dataset.

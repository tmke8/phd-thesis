...
\textsc{Conference}:\;\; Submitted to \textit{Internation Conference on Machine Learning} (ICML), 2021 \\
...
%
This finer-grained subgrouping information is typically unavailable/unlabeled (e.g. \citep{SohDunAngGuetal20,nam2020learning}).
%
...
\begin{figure}[t]
    ...
    \includegraphics[width=0.5\textwidth]{figures/fdm.pdf}
    \caption{%
...
%   \includegraphics[width=\columnwidth]{figures/cmnist_2v4_partial_tnr.pdf}
  \caption{
    Results from 30 repeats for the Colored MNIST dataset with two digits, 2 and 4, with \emph{subgroup bias} for the color `{\color{purple}purple}': for {\color{purple}purple}, only the digit class `2' is present.
    \textsc{Left}: Accuracy.
...
%   \includegraphics[width=\columnwidth]{figures/cmnist_2v4_miss_s_alt_tnr.pdf}
  \caption{
    Results from 30 repeats for the Colored MNIST dataset with two digits, 2 and 4, with a \emph{missing subgroup}: the training dataset only has {\color{green}green} digits.
    \textsc{Left}: Accuracy.
    \textsc{Right}: Positive rate ratio.
    % \textbf{Bottom left}: True positive rate ratio.
    % \textbf{Bottom right}: True negative rate ratio.
...
%   \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_tnr.pdf}
  \caption{
    Results from 30 repeats for the Colored MNIST dataset with three digits: `2', `4' and `6'.
...
\section{Experiments}
We perform experiments using image and tabular datasets: Colored MNIST \citep{kim2019learning}, CelebA \citep{liu2015faceattributes} and Adult Income\footnote{{Results for Adult Income dataset and discussion can be found in the supplementary material.}} \citep{Dua:2017} that are publicly available. 
...
The MNIST dataset \citep{lecun1998gradient} consists of 70,000 (60,000 designated for training, 10,000 for testing) images of gray-scale hand-written digits. We color the digits following a similar procedure to that outlined by \citep{kim2019learning}, randomly assigning each sample one of ten distinct RGB colors. Each source is then a combination of digit-class (class label) and color (subgroup label). We use no data-augmentation aside from symmetrically zero-padding the images to be of size 32x32.

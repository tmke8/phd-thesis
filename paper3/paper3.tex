...
Machine learning has been deployed in safety-critical applications such as medicine (e.g. \citet{DunYiLanReetal19}), and socially important contexts such as the allocation of healthcare, education, and credit (\eg\ \citet{HurAde17,RagBarKleLev20}). 
%
Efficiency can be improved, costs can be reduced, and personalization of services and products can be greatly enhanced -- these are some of the drivers for the widespread development and deployment of machine learning algorithms. 
%
...
%
This finer-grained subgrouping information is typically unavailable/unlabeled (e.g. \citet{SohDunAngGuetal20,NamChaAhnLeeetal20}).
%
The standard training process brings about two inter-connected challenges: a) classifiers often under-perform on important hidden subgroups (\emph{hidden stratification}) \cite{RayDunCarRe20,SohDunAngGuetal20};
and b) \emph{systematic bias}~\cite{KalZho18} affects whether or not entire collections of data points appear in the training dataset,
...
(\emph{residual bias}). %\cite{KalZho18}.
...
applicants who were approved in the past \cite{KalZho18}.
%
...
%
%This systematic bias might lead to the no-patch subgroup to have thin or non-existent labeled data.
%
...
We aim to convert our unlabelled data into a collection of \emph{perfect bags} \citep{KleMulRag16,chouldechova17}, \ie\ sample sets in which the class label $y$ and subgroup label $s$ are independent (\ie\ $y\perp s$).
...
\textbf{On semi-supervised learning.} 
\citet{WicPanTri19} proposed a semi-supervised method that can successfully harness unlabelled data to correct for the selection bias and label bias in the training data.
%
...
\textbf{On disentangled representations learning.}
\citet{LocAbbRaiBauetal19} suggested that disentanglement in representation learning may be a useful property to remove algorithmic bias when subgroup information is not observed.
%
...
%
Though, in many real-world tasks, the variable $s$ is correlated with the variable $y$, and therefore unsupervised methods are not suitable  \citep{JaiWuAbdNat18,JaiWuAbdNat19}. 
%
Indeed, experiments in \citet{LocAbbRaiBauetal19} were wholly done with procedurally generated synthetic datasets involving 2D and 3D shapes. 
%
Without some supervision or inductive bias, disentangled representation methods would not solve the issue of algorithmic fairness with invisible demographics \citep{LacBauLucGeletal19}. 
...
\begin{subfigure}[b]{0.74\textwidth}
    \centering
    %\includegraphics[width=0.9\textwidth]{figures/SSL-framework}
    % \includegraphics[width=\textwidth]{figures/SSL-framework-withPred.pdf}
    \includegraphics[width=\textwidth]{figures/zsf_diagram.pdf}
    \caption{%
...
\begin{subfigure}[b]{0.24\textwidth}
    ...
    \includegraphics[width=\textwidth]{figures/fdm.pdf}
    \caption{%
...
\textbf{Zero-label stratification and algorithmic bias.}
Let $S$ denote a set of discrete-valued subgroup labels of the associated domains $\Scal$.
%
...
%
$X$, with the associated domain $\Xcal$, represents other attributes of the data. % that may or may not be influenced by subgroup labels $S$.
%
Let $\Ycal$ denote the space of class labels for a classification task; $\Ycal = \{0,1\}$ for binary classification or $\Ycal = \{1,2,\ldots,C_{\text{cls}}\}$ for multi-class classification.
%
...
%
As in a standard supervised learning task, we have access to a labelled training set $\Dcal_{tr} =\{(x_i, s_i, y_i)\}$, that is used to learn a model $M:\Xcal \rightarrow \Ycal$. 
$\Dcal_{tr}$ is composed of several sources, but lacks samples from some of the sources:
\begin{align}
\exists y'\in \Ycal,\exists s'\in \Scal: \Dcal_{tr}\cap \Omega_{y=y',s=s'} = \varnothing.
\end{align}
...
(\ie, we have $\Dcal_\mathit{tr} \cap\Omega_{y=1,s=0} = \varnothing$), giving rise to a setting we refer to as \emph{subgroup bias} (SB).
...
%Semi-supervised learning \cite{ChaSchZie06} can alleviate the issue of unfairness to the \emph{invisibles} by mixing labeled with unlabeled data, which is usually much cheaper to obtain. 
%
We propose to alleviate the issue of bias against missing subgroups by mixing labelled data with unlabelled data that is usually much cheaper to obtain \cite{ChaSchZie06}. 
%
In this paper, we refer to this set of \emph{unlabelled} data as the
deployment set\footnote{In our experiments, we report accuracy and bias metrics on another independent test set instead of on the unlabelled data that is available at training time.} $\Dcal_{dep} =\{(x_i)\}$.
%_{i=1}^{N^c}
%
This 
%context 
...
\begin{align}
  \Dcal_{dep} \cap \Omega_{y=y',s=s'} \neq \varnothing\quad\quad\forall y'\in\Ycal,\forall s'\in\Scal~.
\end{align}
...
\textbf{Relation to algorithmic fairness.}
...
\citep{kamiran2012data,HarPriSre16,ZafValRodGum17b,chouldechova17,RagBarKleLev20}
...
\textbf{Perfect bag.} 
We call a sampled set for which $y\perp s$ holds, a perfect bag \citep{chouldechova17,KleMulRag16}.
...
Instead, we pursue learning under zero-label systematic bias as learning disentangled representations with a collection of \emph{approximately} perfect bags. We show that the disentangling procedure is robust enough to work with this relaxation, but that performance scales with how well the deployment set is balanced.

\textbf{Disentangled representation.}
...
%
We can formalise this intuitive definition using group and representation theories \cite{HigAmoPfaRacetal18}, or using structural causal models \cite{SutMilSchBau19}.
%
...
%
Since $s$ is correlated with the class label $y$, we need annotations of the undesired nuisance variable $s$ \cite{JaiRexAbdNat18,JaiWuAbdNat19} to be successful in using disentanglement learning methods for zero-label stratification. %\citet{JaiRexAbdNat18,JaiWuAbdNat19}.
%
We have some annotations of subgroup label $s$ in the training set $\Dcal_{tr} =\{(x_i, s_i, y_i)\}$, however, crucially, due to systematic bias, this set is missing certain subgroups.
%
We have all subgroups in the deployment set $\Dcal_{dep} =\{(x_i)\}$, 
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{figures/celeba_dep_recon.png}
    \caption{%
    Visualization of our method's solutions for the CelebA dataset, with``smiling females'' as the missing subgroup.
...
\textbf{Disentanglement with a collection of perfect bags.} 
Our framework for learning the disentangled representations comprises four core modules: 1) \emph{encoder} functions $f_y$ and $f_s$ (which share weights) that embed $x$ into $z_y$ and $z_s$, respectively; 2) a \emph{decoder} function $g$
 ...
 \begin{align}
   ...
   =&\,\sum_{x \in \mathcal{B}_{tr} \bigcup \gB_\mathit{perf}} L_{\text{recon}} (x,g(f_y(x),f_s(x))) \nonumber\\
     &\,+ \!\!\sum_{x\in \Bcal_{tr}} \lambda_1 L_{\text{sup}} (y, \ell_y(f_y(x))) + \lambda_2 L_{\text{sup}} (s, \ell_s(f_s(x)))\nonumber\\
     &\,- \lambda_3 (
          \log h(\{f_y(x) | x \in \Bcal_\mathit{perf}\}) \nonumber\\
...
\eg, in the case of binary $s$, we restrict $z_s$ to be one-dimensional.
Additionally, $z_s$ can be discretised with a straight-through estimator of the gradient \cite{bengio2013estimating}.

\textbf{Disentanglement guarantees. }
...
As an example, we consider colored MNIST with two classes (digits 2 and 4) and two subgroups (colors purple and green), where the training set is lacking the combination of digit 4 and color purple.
Sample pairs from the two distributions have a shared underlying factor: 2's can be purple and green.
The pairs also have a factor that differs: in the training set, a 4 can only be green; in the deployment set, it can have both colors.
Theorem 1 in \cite{ShuCheKumErmetal20} guarantees then that a (sufficiently powerful) disentangling encoder will produce a disentangling that is \emph{consistent} with respect to the shared factor and \emph{restricted} with respect to the changing factor.
...
\begin{figure*}[ht]
  ...
  \includegraphics[width=\columnwidth]{figures/cmnist_2v4_partial_prr.pdf}
...
%   \includegraphics[width=\columnwidth]{figures/cmnist_2v4_partial_tnr.pdf}
  \caption{
    Results from \textbf{30 repeats} for the Colored MNIST dataset with two digits, 2 and 4, with \emph{subgroup bias} for the color `{\color{purple}purple}': for {\color{purple}purple}, only the digit class `2' is present.
    \textbf{Left}: Accuracy.
...
\begin{figure*}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures/cmnist_2v4_miss_s_acc.pdf}
  \includegraphics[width=\columnwidth]{figures/cmnist_2v4_miss_s_prr.pdf}
...
%   \includegraphics[width=\columnwidth]{figures/cmnist_2v4_miss_s_alt_tnr.pdf}
  \caption{
    Results from \textbf{30 repeats} for the Colored MNIST dataset with two digits, 2 and 4, with a \emph{missing subgroup}: the training dataset only has {\color{green}green} digits.
    \textbf{Left}: Accuracy.
    \textbf{Right}: Positive rate ratio.
    % \textbf{Bottom left}: True positive rate ratio.
    % \textbf{Bottom right}: True negative rate ratio.
...
\begin{figure*}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_acc.pdf}
  \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_hgr.pdf}
%   \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_pr.pdf}
%   \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_tpr.pdf}
%   \includegraphics[width=\columnwidth]{figures/cmnist_3dig_4miss_tnr.pdf}
  \caption{
    Results from \textbf{30 repeats} for the Colored MNIST dataset with three digits: `2', `4' and `6'.
    Four combinations of digit and color are missing: {\color{green}green} 2's, {\color{blue}blue} 2's, {\color{blue}blue} 4's and {\color{green}green} 6's.
    \textbf{Left}: Accuracy.
    \textbf{Right}: Hirschfeld-Gebelein-R\'enyi maximal correlation \citep{renyi1959measures} between $S$ and $Y$.
    % \textbf{Right}: Positive rate ratio.
    % \textbf{Bottom left}: True positive rate ratio.
    % \textbf{Bottom right}: True negative rate ratio.
  }%
...
\begin{figure*}[ht]
  ...
  \includegraphics[width=\columnwidth]{figures/celeba_gender_smiling_prr.pdf}
%   \includegraphics[width=\columnwidth]{figures/celeba_gender_smiling_tpr.pdf}
%   \includegraphics[width=\columnwidth]{figures/celeba_gender_smiling_tnr.pdf}
  \caption{
...
Our proposed framework, demonstrated in fig.\,\ref{fig:framework}, entails two steps: 1) sample perfect bags from an unlabeled deployment set, and 2) produce disentangled representations using perfect bags for adversarial distribution-matching.

\textbf{Constructing approximately perfect bags via clustering.}
...
% %
% Since the deployment set is unlabeled, $s_{ij}$ are unknown. 
% %
...
% %
% In addition to this unsupervised training, the classifier $C$ is also trained on the labeled data using the standard cross-entropy loss.  
% %We train the classifier $C$ on the training data using a standard cross-entropy loss and the labeled sources $\Omega_{y,s}$. 
% %
...
% %
% If the rankings obtained for two unlabeled data points $z_i$ and $z_j$ are the same, they are assigned to the same class, $s_{ij}=1$. 
% %
As a result of clustering, the data points in the deployment set $\Dcal_\mathit{dep}$ are labeled with cluster assignments 
$\Dcal_\mathit{dep}=\{(x_i, c_i)\}$, $c_i = C(z_i)$.
%
We balance $\Dcal_\mathit{dep}$ so that all clusters have equal size to form a perfect bag
...
%
We can subsequently apply the union bound to take into account that we have a fully-unlabeled setting.
...
%
Although having zero labeled examples for some subgroups is not uncommon due to the effects of systematic bias,
we should make a value-judgment on the efficacy of the dataset with respect to a task.
%
...
%
% In the case of 2-digit Colored MNIST, for example, the deployment set may contain only a small portion of {\color{purple}purple} fours relative to the other subgroups, meaning that the cluster they form can be easily overlooked by a clustering algorithm in favor of larger but less salient clusters (which may be sub-clusters of other digit/color combinations). 
The cluster formed by an underrepresented subgroup can be easily overlooked by a clustering algorithm in favor of larger but less salient clusters
...
%Clearly, the use of algorithmic approaches still requires human review in a manner that is similar to error auditing, but is less dependent on the specific human auditor to initially identify the stratifica- tion. While algorithmic approaches to measurement can reduce burden on human analysts and take advantage of learned encodings to identify subclasses, their efficacy is limited by the difficulty of separating important subclasses in the feature space analyzed.

\section{Experiments}
We perform experiments using image and tabular datasets: Colored MNIST \citep{kim2019learning}, CelebA \cite{liu2015celeba} and Adult Income\footnote{{Results for Adult Income dataset and discussion can be found in the supplementary material.}} \citep{Dua:2017} that are publicly available. 
...
3) without balancing, when the deployment set $\Dcal_{dep}$ is used as is (\texttt{No bal.});
...
The second of the aforementioned baselines is \texttt{DRO} \citep{HasSriNamLia18}, which functions without subgroup labels by minimizing the worst-case training loss over all possible groups that are above a certain minimum size;
...
%
%%%%%OR
%During training, the {\color{green}green} color group is observed in both digits (sources $\Omega_{y=\text{'two' }s=\color{green}green}$ and $\Omega_{y=\text{'four' }s=\color{green}green}$), but for the {\color{purple}purple} color group, we only observe the one-sided outcome ($\Omega_{y=\text{'two'}s=\color{purple}purple}$), so the source $\Omega_{y=\text{'four'}s=\color{green}green}$ defines invisible demographic.  
%
\textbf{Colored MNIST.}
The MNIST dataset \cite{lecun1998gradient} consists of 70,000 (60,000 designated for training, 10,000 for testing) images of gray-scale hand-written digits. We color the digits following a similar procedure to that outlined by \cite{kim2019learning}, randomly assigning each sample one of ten distinct RGB colors. Each source is then a combination of digit-class (class label) and color (subgroup label). We use no data-augmentation aside from symmetrically zero-padding the images to be of size 32x32.
...
We begin by considering a binary, 2-digit, 2-color, variant of the dataset with $Y = \{2, 4\}$ and $S = \{\text{\color{green}green}, \text{\color{purple}purple}\}$.
...
Again we see that poor clustering (in the case of \texttt{K-means}) is detrimental to the disentangling procedure, though for all balancing strategies the IQR is significantly higher than observed in the MS setting, along with there being a number of extreme outliers. The median, however, remains high, which is reflective of the ``hit-or-miss'' performance of the method under these conditions, but where the number of hits far outweighs the number of misses. Visualization of the reconstructions with $z_y$ zeroed-out suggest that misses often mostly occurred due to the semantic information being concentrated in $z_s$ while $z_y$ is left to contain only residual information, even when $z_s$ was set to be one-dimensional and binarised. We leave it to future work to explore how to better offset such degeneracies.

To investigate how an increase in the number of classes affects disentangling of classes and groups, we look to a 3-digit, 3-color variant of the dataset in the SB setting where four sources are missing from $D_{tr}$.
...
\textbf{CelebA.} To demonstrate our method can be used to mitigate biased decision making for real-world computer vision problems, we consider the CelebA dataset~\cite{liu2015celeba} comprising over 200,000 images of different celebrities.
...
%
Since the dataset exhibits natural imbalance in $S \times Y$, we perform no additional sub-sampling of either the training set or the deployment set, as we did for Colored MNIST.
%
...
%
\section{Conclusion}
We have highlighted the problem that systematic bias can result in one or more subgroups having zero labeled data, and by doing so hope to have stimulated serious consideration for it (even if to be dismissed) when planning, building, evaluating and regulating machine learning systems.
We propose a two-step approach for addressing the resulting zero-label stratification problem.
First, we construct perfect bags from an unlabeled deployment set via  clustering. 
Second, we learn a disentangled representations using the perfect bags for adversarial distribution-matching. 
We empirically validate our framework on the Colored MNIST, CelebA and Adult Income datasets, and find evidence that it is possible to maintain high performance on the subgroups with zero training data. 
We analyze our approach using the disentanglement calculus of \citet{ShuCheKumErmetal20} that relies on the notions of restrictiveness and consistency, and show that we can derive some guarantees.
